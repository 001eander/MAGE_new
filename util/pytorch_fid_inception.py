"""pytorch-fid InceptionV3 (TF 2015-12-05 weights).

Copied from mseitzer/pytorch-fid so eval matches LGANs-TT realdata.py:
    from pytorch_fid import fid_score
    fid_score.calculate_fid_given_paths([real, fake], 50, device, 2048)
"""
import os
...
def _load_fid_state_dict():
    for path in _LOCAL_FID_WEIGHTS:
        if os.path.isfile(path):
            return torch.load(path, map_location='cpu')
    return load_state_dict_from_url(FID_WEIGHTS_URL, progress=True)
...
def _inception_v3(*args, **kwargs):
    ...
    if version < (0, 13) and 'weights' in kwargs:
        if kwargs['weights'] == 'DEFAULT':
            ...
            raise ValueError('weights=={} not supported'.format(kwargs['weights']))
        del kwargs['weights']
...
def fid_inception_v3():
    ...
    inception.load_state_dict(_load_fid_state_dict())
    return inception
